...
func tabs*(titles: openArray[string], selected = 0, highlightStyle = defaultStyle(),
           divider = " | ", blk = none(Block)): Tabs =
  ## Construct a tab bar.
  Tabs(titles: @titles, selected: selected, highlightStyle: highlightStyle,
       divider: divider, blk: blk)
...
proc render*(t: Tabs, area: Rect, buf: var Buffer) =
  ...
  for i, title in t.titles:
    ...
    let st = if i == t.selected: t.style.patch(t.highlightStyle) else: t.style

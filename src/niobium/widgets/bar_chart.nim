...
func barChart*(bars: openArray[Bar], barWidth = 1, barGap = 1, max = none(int),
               barStyle = defaultStyle(), blk = none(Block)): BarChart =
  ## Construct a bar chart.
  BarChart(bars: @bars, barWidth: barWidth, barGap: barGap, max: max,
           barStyle: barStyle, blk: blk)
...
proc render*(c: BarChart, area: Rect, buf: var Buffer) =
  ...
  let hasLabels = block:
    var any = false
    for b in c.bars:
      if b.label.len > 0: any = true
    any
  let chartBottom = if hasLabels: inner.bottom - 1 else: inner.bottom
  let chartHeight = chartBottom - inner.top
  if chartHeight <= 0: return

  var maxVal = if c.max.isSome: c.max.get else: 0
  if c.max.isNone:
    for b in c.bars: maxVal = max(maxVal, b.value)
  if maxVal <= 0: return
  ...
  for b in c.bars:
    ...
    for col in 0 ..< c.barWidth:
      let bx = x + col
      if bx >= inner.right: break
      for r in 0 ..< fullRows:
        buf.setStringN(bx, chartBottom - 1 - r, "█", c.barStyle.patch(b.style), 1)
      if partial > 0 and fullRows < chartHeight:
        buf.setStringN(bx, chartBottom - 1 - fullRows, eighthsUp[partial - 1],
                       c.barStyle.patch(b.style), 1)

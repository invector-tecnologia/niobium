...
proc drawLine*(buf: var Buffer, x, y: int, ln: Line, maxWidth: int, base: Style): int {.discardable.} =
...
func alignOffset*(align: Alignment, width, content: int): int =
  ...
  case align
  of alLeft: 0
  of alCenter: max(0, (width - content) div 2)
  of alRight: max(0, width - content)

...
func dataset*(name: string, data: openArray[tuple[x, y: float]], marker = mkDot,
             graphType = gtScatter, style = defaultStyle()): Dataset =
  Dataset(name: name, data: @data, marker: marker, graphType: graphType, style: style)

func chart*(datasets: openArray[Dataset], xBounds, yBounds: tuple[lo, hi: float],
            blk = none(Block)): Chart =
...
proc plot(buf: var Buffer, area: Rect, x, y: float, xb, yb: tuple[lo, hi: float],
          sym: string, st: Style) =
  if xb.hi <= xb.lo or yb.hi <= yb.lo: return
  if x < xb.lo or x > xb.hi or y < yb.lo or y > yb.hi: return
  let px = area.left + int((x - xb.lo) / (xb.hi - xb.lo) * float(area.width.int - 1) + 0.5)
  let py = area.bottom - 1 - int((y - yb.lo) / (yb.hi - yb.lo) * float(area.height.int - 1) + 0.5)
    ...
    if ds.graphType == gtLine and ds.data.len >= 2:
      for i in 0 ..< ds.data.high:
        ...
        for s in 0 .. steps:
          ...
          plot(buf, inner, a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t,
               c.xBounds, c.yBounds, sym, ds.style)
    else:
      for p in ds.data:

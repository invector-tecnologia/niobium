...
func table*(rows: openArray[Row], widths: openArray[Constraint], header = none(Row),
            columnSpacing = 1, blk = none(Block), style = defaultStyle(),
            highlightStyle = defaultStyle(), highlightSymbol = ""): Table =
  ## Construct a table.
  Table(rows: @rows, widths: @widths, header: header, columnSpacing: columnSpacing,
        blk: blk, style: style, highlightStyle: highlightStyle, highlightSymbol: highlightSymbol)
...
proc render*(t: Table, area: Rect, buf: var Buffer, state: var TableState) =
  ...
  if t.header.isSome:
    ...
    let hcols = rect(gridLeft, y, gridWidth, 1).split(Horizontal, t.widths, spacing = t.columnSpacing)
  ...
  if state.selected.isSome:
    ...
    elif sel >= state.offset + rows: state.offset = sel - rows + 1
  if state.offset < 0: state.offset = 0

  var i = state.offset
  while i < t.rows.len and y < inner.bottom:
    let selected = state.selected.isSome and state.selected.get == i
    let st = if selected: t.style.patch(t.rows[i].style).patch(t.highlightStyle)
             else: t.style.patch(t.rows[i].style)
    if symWidth > 0:
      let prefix = if selected: t.highlightSymbol else: " ".repeat(symWidth)
      buf.setStringN(inner.left, y, prefix, st, symWidth)
    let rrect = rect(inner.left, y, inner.width.int, 1)
    let rcols = rect(gridLeft, y, gridWidth, 1).split(Horizontal, t.widths, spacing = t.columnSpacing)

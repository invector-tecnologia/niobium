...
proc render*(s: Scrollbar, area: Rect, buf: var Buffer, state: ScrollbarState) =
  ...
  if area.isEmpty or state.contentLength <= 0:
    return
  ...
  var thumbLen =
    max(1, (state.viewportContentLength * trackLen) div max(1, state.contentLength))
  ...
  if vertical:
    let x =
      if s.orientation == soVerticalRight:
        area.right - 1
      else:
        area.left
    for i in 0 ..< trackLen:
      ...
      buf.setStringN(
        x,
        y,
        (if inThumb: s.thumbSymbol else: s.trackSymbol),
        (if inThumb: s.thumbStyle else: s.trackStyle),
        1,
      )
  else:
    let y =
      if s.orientation == soHorizontalBottom:
        area.bottom - 1
      else:
        area.top
    for i in 0 ..< trackLen:

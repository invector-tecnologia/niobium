...
func initBlock*(title = "", borders: Borders = {}, borderType = btPlain,
                borderStyle = defaultStyle(), style = defaultStyle(),
                titleAlignment = alLeft, titleStyle = defaultStyle(),
                padding = Padding()): Block =
  ## Construct a block. With no borders it is just an optional background + padding.
  Block(title: title, borders: borders, borderType: borderType, borderStyle: borderStyle,
        style: style, titleAlignment: titleAlignment, titleStyle: titleStyle, padding: padding)
...
func inner*(b: Block, area: Rect): Rect =
  ...
  if sLeft in b.borders: inc l
  if sTop in b.borders: inc t
  if sRight in b.borders: dec r
  if sBottom in b.borders: dec bot
...
proc render*(b: Block, area: Rect, buf: var Buffer) =
  ...
  if sTop in b.borders:
    for x in area.left ..< area.right: buf.setStringN(x, area.top, g.h, bs, 1)
  if sBottom in b.borders:
    for x in area.left ..< area.right: buf.setStringN(x, area.bottom - 1, g.h, bs, 1)
  if sLeft in b.borders:
    for y in area.top ..< area.bottom: buf.setStringN(area.left, y, g.v, bs, 1)
  if sRight in b.borders:
    for y in area.top ..< area.bottom: buf.setStringN(area.right - 1, y, g.v, bs, 1)
  if {sTop, sLeft} <= b.borders: buf.setStringN(area.left, area.top, g.tl, bs, 1)
  if {sTop, sRight} <= b.borders: buf.setStringN(area.right - 1, area.top, g.tr, bs, 1)
  if {sBottom, sLeft} <= b.borders: buf.setStringN(area.left, area.bottom - 1, g.bl, bs, 1)
  if {sBottom, sRight} <= b.borders: buf.setStringN(area.right - 1, area.bottom - 1, g.br, bs, 1)

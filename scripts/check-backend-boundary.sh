#!/usr/bin/env bash
# Enforces pillar 2 (backend agnosticism): only src/niobium/backend/ may write to the
...
violations="$(grep -REn "$pattern" src --include='*.nim' \
  | grep -v '^src/niobium/backend/' || true)"

if [[ -n "$violations" ]]; then
  echo "Backend boundary violation: terminal output outside src/niobium/backend/:"
  echo "$violations"
  exit 1
fi

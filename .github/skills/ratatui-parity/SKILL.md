---
name: ratatui-parity
description: "Use when porting any widget, layout, buffer, or backend behavior from ratatui to Niobium. Covers the parity-map workflow, semantic-diff checks, and which Rust idioms NOT to copy (traits, lifetimes, CompactString, macros). Triggers: port, parity, ratatui equivalent, faithful, mirror behavior."
---

# ratatui parity

Niobium mirrors ratatui's behavior. This skill governs how to port a construct faithfully.
...
2. Locate or add the row in `docs/reference/parity-map.md`:
   `ratatui symbol → niobium symbol → status → ADR/spec link`.
...
- `CompactString` cell symbol → Niobium's SSO rune storage (ADR-0003); keep the API, not the type.
